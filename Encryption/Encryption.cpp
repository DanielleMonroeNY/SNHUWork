...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { // TODO: student need to change the next line from output[i] = source[i]
   ...
   //CHANGED: changed each incoming character with an xor
...
std::string read_file(const std::string& filename)
{
  ...
  //CHANGED: The following sections were added up until the return statement
  //Variables to hold neccessary fields
  std::fstream inputDataFile(filename);
  std::int64_t fSize = sizeof(inputDataFile);
  std::ostringstream dataIntoStr;

  //if/else statement to check for buffer overflowdetermine if a file has opened properly 
  if (!inputDataFile.is_open())
  {
    std::cout << "File Can not be found" << std::endl;

  }else{
    //data from file to std::ostringstream then casted into a string
    dataIntoStr << inputDataFile.rdbuf();
    file_text = dataIntoStr.str();
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //CHANGED: Created variables and formatted data to be saved to the file. 
  std::string sName, k, d;
  sName = student_name;
  k = key;
  d = data;
  std::fstream fName (filename);

  //Date formatting
  char dateDest[100];
  std::size_t dateCharLim = sizeof(dateDest);
  //char dateFormat[std::size_t("YYYY-mm-dd")];
  std::time_t date = std::strftime(dateDest, dateCharLim, "%Y %m %d", std::localtime(&date));
